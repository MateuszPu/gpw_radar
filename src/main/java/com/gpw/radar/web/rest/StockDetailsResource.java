...
public class StockDetailsResource {
     ...
     */
    @RequestMapping(value = "/stockDetailss",
    ...
    @RequestMapping(value = "/stockDetailss",
            ...
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<StockDetails>> getAll(@RequestParam(value = "page" , required = false) Integer offset,
     ...
     */
    @RequestMapping(value = "/stockDetailss/{id}",
            method = RequestMethod.GET,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<StockDetails> get(@PathVariable Long id) {
        log.debug("REST request to get StockDetails : {}", id);
        return Optional.ofNullable(stockDetailsRepository.findOne(id))
            .map(stockDetails -> new ResponseEntity<>(
                stockDetails,
                HttpStatus.OK))
