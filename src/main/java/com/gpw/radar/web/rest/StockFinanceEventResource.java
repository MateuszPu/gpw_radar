...
public class StockFinanceEventResource {
     ...
     */
    @RequestMapping(value = "/stockFinanceEvents",
    ...
    @RequestMapping(value = "/stockFinanceEvents",
            ...
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<StockFinanceEvent>> getAll(@RequestParam(value = "page" , required = false) Integer offset,
     ...
     */
    @RequestMapping(value = "/stockFinanceEvents/{id}",
            method = RequestMethod.GET,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<StockFinanceEvent> get(@PathVariable Long id) {
        log.debug("REST request to get StockFinanceEvent : {}", id);
        return Optional.ofNullable(stockFinanceEventRepository.findOne(id))
            .map(stockFinanceEvent -> new ResponseEntity<>(
                stockFinanceEvent,
                HttpStatus.OK))
